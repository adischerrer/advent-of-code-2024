...
def path_cost(path: list[tuple[int, int]]) -> int:
    cost: int = 0
    last_direction: tuple[int, int] = (0, 0)
    for i in range(1, len(path)):
        direction: tuple[int, int] = path[i][0] - path[i - 1][0], path[i][1] - path[i - 1][1]
...
def search(maze: Maze, path: list[tuple[int, int]], params: Params) -> None:
    if path[-1] == maze.end_pos:
        if path_cost(path) < path_cost(params.best_path) or len(params.best_path) == 0:
            params.best_path = path.copy()
            print(f'New best path: Cost: {path_cost(params.best_path)}')
        return
    if not len(params.best_path) == 0 and path_cost(path) > path_cost(params.best_path):
        return
...
def main():
    ...
    print_maze(maze)
    print(f'Result: Cost: {path_cost(params.best_path)}')
